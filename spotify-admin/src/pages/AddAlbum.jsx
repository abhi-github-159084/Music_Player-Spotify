...
function AddAlbum() {
  ...
  const onSubmitHandler = async (e) => {
    // whenever we submit the form it will keep reloading the webpage so not to done so use this
    e.preventDefault();
    setLoading(true);
    try{
  ...
  return loading ? (
    ...
    <form onSubmit={onSubmitHandler} className=' flex flex-col items-start gap-8 text-gray-800'>
      ...
      <div className=' flex flex-col gap-4 '>
        <p>Upload Image</p>
        <input onChange={(e)=> {console.log(e);setImage(e.target.files[0])}} type='file' id='image' accept='image/*' hidden/>
        <label htmlFor='image'>
